...
class ERAGEMPlugin(StrategyPlugin):
    """
    ...
    def before_forward(self, strategy, **kwargs):
        ...
        if x_s is not None:  
            ...
            x_s = x_s.to(strategy.device)
            y_s = y_s.to(strategy.device)
            out = avalanche_forward(strategy.model, x_s, t_s)
            loss = strategy._criterion(out, y_s)
    ...
    def before_backward(self, strategy, **kwargs):
        ...
        if strategy.clock.train_exp_counter > 0:
            ...
            strategy.loss *= (self.lmbda*2) * self.lmbda_weighting
        return
